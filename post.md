...
as painless end automatic as possible.
...
Write posts in Markdown is nice but is not very useful if you still have to
...
So the script will initiate a OAuth negotiation when needed and store the
...
Before we're able to use this wee need to create a new *API project*, configure
...
Will push the post to Blogger, so we're done (?)
